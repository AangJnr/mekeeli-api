...
@router.get("/users/", response_model=list[schemas.User], dependencies=[Depends(security.get_current_admin_user)])
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_users(db, skip=skip, limit=limit)
    return users

@router.get("/users/{user_id}", response_model=schemas.User, dependencies=[Depends(security.get_current_admin_user)])
