...
async def get_mcp_api_key(db: Session, user_id: int, mcp_name: str) -> str | None:
    """Retrieves the API key for a given MCP from the user's settings."""
    # MCP API keys are stored in the format 'mcp_apikey_{mcp_name}'
    api_key_setting = crud.get_user_setting(db, user_id=user_id, key=f"mcp_apikey_{mcp_name}")
    if api_key_setting:
